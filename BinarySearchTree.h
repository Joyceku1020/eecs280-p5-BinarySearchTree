...
private:
  ...
  static Node *min_greater_than_impl(Node *node, const T &val, Compare less)
  {
    ...
    if (less(val, node->datum) && (node->left == nullptr || less(node->left->datum, val) || (!less(node->left->datum, val) && !less(val, node->left->datum))))
    {
      return node;
    }
    ...
    if (min_element_impl(node -> right) == node -> right && less(node -> datum, (node -> right) -> datum)) {
      return node -> right;
    }

    if ((!less(max_element_impl(node -> left) -> datum, val) && !less(val, max_element_impl(node -> left) -> datum)) ||
    less(max_element_impl(node -> left) -> datum, val)) {
      return node;
    }
